...
import { FaRegCheckSquare, FaCheckCircle } from "react-icons/fa";
...
function ProjectDetails() {
  ...
  return (
    <div className="mt-5 flex flex-col mb-5 gap-2">
      ...
      {/* Starting Project */}
      <article className="flex flex-wrap justify-center gap-1 mt-5 items-stretch">
        {/* Left Panel */}
        <section className="flex flex-3 flex-col border border-gray-300">
          <img src={project.image} alt={`${project.title} Image`} />

          <div className="flex">
            {/* Category, Title, Description */}
            <div className="flex flex-3 flex-col justify-start items-start m-10 gap-5">
            ...
            <div className="flex flex-1 flex-col m-5 gap-5">
              <div>
        ...
        {/* Right Panel */}
        <section className="flex flex-2 flex-col justify-start border border-gray-300 p-3 gap-2">
          {/* Gallery */}
          <h2 className="font-bold text-2xl">Gallery</h2>
          <div className="grid md:grid-cols-1 lg:grid-cols-2 grid-rows-2 gap-1 border border-gray-200 flex-1">
            {project.gallery.map((img, idx) => (
              <img
                key={idx}
                src={img}
                alt="Image"
                className={`w-full h-full object-cover ${idx === 2 && project.gallery.length === 3 ? "row-span-2 col-start-1 row-start-1" : ""}`}
              />
            ))}
          </div>
          ...
          <div className="flex flex-col flex-1">
            {project.features.map((feature, idx) => (
              <div className="space-x-2 flex items-center" key={idx}>
                <span className="font-mono text-[#1d3557] font-bold">
